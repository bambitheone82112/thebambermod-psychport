	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['Bamber edition by'], 15);
					#else
					createCoolText(['slxickey', 'manny', 'stopbansan', 'tajeem', 'hortas']);
					#end
				// credTextShit.visible = true;
				case 4:
					#if PSYCH_WATERMARKS
					addMoreText('slxickey', 15);
					addMoreText('manny', 15);
					addMoreText('stopbansan', 15);
					#else
					addMoreText('present');
					#end
				// credTextShit.text += '\npresent...';
